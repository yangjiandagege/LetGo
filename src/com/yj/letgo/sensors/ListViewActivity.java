...
public class ListViewActivity extends Activity {
	...
	private SensorManager sensorManager; // 传感器的管理类
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ArrayList<Sensor> tempSensors=new ArrayList<Sensor>(); //由于List是一个接口，方法没有实现
		for(Sensor s:list){
			tempSensors.add(s);
		}
		...
		for(int i =0;i<tempSensors.size();i++){
			...
			if (!(string.toLowerCase().contains("accelerometer")
					|| string.toLowerCase().contains("acceleration")
					|| string.toLowerCase().contains("orientation")
					|| string.toLowerCase().contains("magnetic")
					|| string.toLowerCase().contains("magnetometer")
					...
					|| string.toLowerCase().contains("linear acceleration")
		...
		for (int i = 0; i < tempSensors.size(); i++) {
			String string = ((Sensor) tempSensors.get(i)).getName();
			 if (string.toLowerCase().contains("rotation")) {
					sensorName[i] = "旋转矢量传感器";
			}else if (string.toLowerCase().contains("accelerometer")||string.toLowerCase().contains("acceleration")) {
				sensorName[i] = "加速度传感器";
			} else if (string.toLowerCase().contains("orientation")) {
				sensorName[i] = "方向传感器";
			} else if (string.toLowerCase().contains("magnetic")||string.toLowerCase().contains("magnetometer")) {
				sensorName[i] = "磁力传感器";
				...
				sensorName[i] = "近距感应传感器";
			} else if (string.toLowerCase().contains("gravity")) {
				sensorName[i] = "重力传感器";
			} else if (string.toLowerCase().contains("linear acceleration")) {
		...
		for (int i = 0; i < tempSensors.size(); i++) {
			...
			listItemAdapter = new SimpleAdapter(this, listItem,// 数据源
					R.xml.sensors_list_item,// ListItem的XML实现
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				if (string.toLowerCase().contains("linear acceleration")) {
